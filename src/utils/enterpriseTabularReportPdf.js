...
const DEFAULT_FOOTER = `Generated by ${BRAND_COMPANY_NAME} ERP`;
...
export async function drawEnterprisePdfHeader(doc, layout, origin, meta, pdfFont) {
  ...
  let rightY = 11;
  doc.text(`Generated  ·  ${meta.generatedLabel}`, rightX, rightY, { align: "right" });
  rightY += 4.5;
  if (meta.reportId) {
    doc.text(`Report ID  ·  ${meta.reportId}`, rightX, rightY, { align: "right" });
    rightY += 4.5;
  }
  if (meta.preparedBy) {
    doc.text(`Prepared by  ·  ${meta.preparedBy}`, rightX, rightY, { align: "right" });
    rightY += 4.5;
  }
  (meta.rightLines || []).forEach((line) => {
...
export function drawEnterprisePdfMetaStrip(doc, layout, startY, filterLines, branch, pdfFont) {
...
export function drawEnterprisePdfKpiCards(doc, layout, startY, cards, pdfFont) {
  const { margin, contentW } = layout;
...
export async function buildEnterpriseTabularReportPdf(payload) {
  ...
  let y = await drawEnterprisePdfHeader(doc, layout, origin, {
    title,
    subtitle,
    generatedLabel,
  ...
  y = drawEnterprisePdfMetaStrip(doc, layout, y, filterLines, reportMeta.branch || reportMeta.center, pdfFont);
